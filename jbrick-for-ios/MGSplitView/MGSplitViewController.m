...
- (void)setup
{
	...
	_splitPosition = 280;//MG_DEFAULT_SPLIT_POSITION;
	...
	_dividerViewMD = [[MGSplitDividerView alloc] initWithFrame:divRect];
	_dividerViewMD.splitViewController = self;
	_dividerViewMD.backgroundColor = [UIColor clearColor];
    
    _dividerViewDP = [[MGSplitDividerView alloc] initWithFrame:divRect];
	_dividerViewDP.splitViewController = self;
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    [self.propertyViewController willRotateToInterfaceOrientation:toInterfaceOrientation duration:duration];
}
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation
{
    ...
    [self.propertyViewController didRotateFromInterfaceOrientation:fromInterfaceOrientation];
}
										 ...
										 duration:(NSTimeInterval)duration
{
    ...
    [self.propertyViewController willAnimateRotationToInterfaceOrientation:toInterfaceOrientation duration:duration];
...
- (void)willAnimateFirstHalfOfRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    [self.propertyViewController willAnimateFirstHalfOfRotationToInterfaceOrientation:toInterfaceOrientation duration:duration];
}
...
- (void)didAnimateFirstHalfOfRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation
{
    ...
    [self.propertyViewController didAnimateFirstHalfOfRotationToInterfaceOrientation:toInterfaceOrientation];
}
...
- (void)willAnimateSecondHalfOfRotationFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    [self.propertyViewController willAnimateSecondHalfOfRotationFromInterfaceOrientation:fromInterfaceOrientation duration:duration];
}
...
- (void)layoutSubviewsForInterfaceOrientation:(UIInterfaceOrientation)theOrientation withAnimation:(BOOL)animate
{
	...
	if ([self isVertical]) {
		...
		CGRect masterRect, dividerRectMD, detailRect, dividerRectDP, propertyRect;
		if (masterFirst) {
			if (!shouldShowMaster) {
				// Move off-screen.
			...
			newFrame.origin.x += newFrame.size.width - _splitWidth;
			newFrame.size.width = _splitWidth;
			dividerRectMD = newFrame;
			
			newFrame.origin.x += newFrame.size.width;
			newFrame.size.width = width - _splitPosition;
			detailRect = newFrame;
            
            newFrame.origin.x += newFrame.size.width - _splitWidth;
			newFrame.size.width = _splitWidth;
			dividerRectDP = newFrame;
            
            newFrame.origin.x += newFrame.size.width;
            newFrame.size.width = _splitPosition;
            propertyRect = newFrame;
		...
		// Position divider.
		theView = _dividerViewMD;
		theView.frame = dividerRectMD;
		if (!theView.superview) {
			[self.view addSubview:theView];
		}
        ...
        // Position divider.
		theView = _dividerViewDP;
		theView.frame = dividerRectDP;
		if (!theView.superview) {
			[self.view addSubview:theView];
		}
        [self.view bringSubviewToFront:theView];
        ...
        // Position property
        controller = self.propertyViewController;
		if (controller && [controller isKindOfClass:[UIViewController class]])  {
			theView = controller.view;
			if (theView) {
				theView.frame = propertyRect;
				if (!theView.superview) {
					[self.view insertSubview:theView aboveSubview:self.detailViewController.view];
				} else {
					[self.view bringSubviewToFront:theView];
				}
			}
	...
	if(_dividerStyle != MGSplitViewDividerStyleShadow){
        ...
        for(UIView *view in _cornerViews)
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if ([self isShowingMaster]) {
        ...
        [self.propertyViewController viewWillAppear:animated];
    }
...
- (void)viewDidAppear:(BOOL)animated
{
	...
	if ([self isShowingMaster]) {
        ...
        [self.propertyViewController viewDidAppear:animated];
    }
...
- (void)viewWillDisappear:(BOOL)animated
{
	...
	if ([self isShowingMaster]) {
        ...
        [self.propertyViewController viewWillDisappear:animated];
    }
...
- (void)viewDidDisappear:(BOOL)animated
{
	...
	if ([self isShowingMaster]) {
        ...
        [self.propertyViewController viewDidDisappear:animated];
    }
...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context
{
	if (([animationID isEqualToString:MG_ANIMATION_CHANGE_SPLIT_ORIENTATION] || 
		...
		_dividerViewMD.hidden = NO;
        _dividerViewDP.hidden = NO;
	}
}
...
- (IBAction)toggleSplitOrientation:(id)sender
{
	...
	if (showingMaster) {
		if (_cornerViews) {
			...
			_dividerViewMD.hidden = YES;
            _dividerViewDP.hidden = YES;
		}
...
- (UIViewController *)propertyViewController
{
...
- (void)setPropertyViewController:(UIViewController *)property
{
	if (!_viewControllers) {
		_viewControllers = [[NSMutableArray alloc] initWithCapacity:3];
		[_viewControllers addObject:[NSNull null]];
	...
	if ([_viewControllers count] > 2) {
		if ([_viewControllers objectAtIndex:2] == property) {
			changed = NO;
		} else {
			[_viewControllers replaceObjectAtIndex:2 withObject:property];
		}
		
	} else {
		[_viewControllers addObject:property];
	}
...
- (MGSplitDividerView *)dividerViewMD
{
	return _dividerViewMD ;
}


- (void)setDividerViewMD:(MGSplitDividerView *)divider
{
	if (divider != _dividerViewMD) {
		[_dividerViewMD removeFromSuperview];
		_dividerViewMD.splitViewController = self;
		_dividerViewMD.backgroundColor = MG_DEFAULT_CORNER_COLOR;
		if ([self isShowingMaster]) {
			[self layoutSubviews];
		}
	}
...
- (MGSplitDividerView *)dividerViewDP
{
	return _dividerViewDP ;
}


- (void)setDividerViewDP:(MGSplitDividerView *)divider
{
	if (divider != _dividerViewDP) {
		[_dividerViewDP removeFromSuperview];
		_dividerViewDP.splitViewController = self;
		_dividerViewDP.backgroundColor = MG_DEFAULT_CORNER_COLOR;
		if ([self isShowingMaster]) {
			[self layoutSubviews];
		}
	}
...
{
	if (_dividerViewMD) {
		return _dividerViewMD.allowsDragging;
	}
    if (_dividerViewDP) {
		return _dividerViewDP.allowsDragging;
	}
...
{
	if (self.allowsDraggingDivider != flag && _dividerViewMD) {
		_dividerViewMD.allowsDragging = flag;
	}
    if (self.allowsDraggingDivider != flag && _dividerViewDP) {
		_dividerViewDP.allowsDragging = flag;
	}
}
...
- (void)setDividerStyle:(MGSplitViewDividerStyle)newStyle
{
	...
	[_dividerViewMD setNeedsDisplay];
    [_dividerViewDP setNeedsDisplay];
	if (_cornerViews) {
...
@synthesize propertyViewController;
@synthesize dividerViewMD;
@synthesize dividerViewDP;
